...
"""
# optuna - for hyperprameter tuning with CV 
...
IRRELEVANT_FEATURES = [ 'nateglinide','chlorpropamide','tolbutamide','acarbose','miglitol','troglitazone',
 ...
 def objective_cb(trial):
    ...
    cb_params = {
        ...
        #'subsamples':trial.suggest_float('subsamples', 0.5, 1),
...
study = optuna.create_study(direction="maximize")
study.optimize(objective_cb, timeout=400)
print('Best hyperparameters:', study.best_params)
print('Best balance accuracy:', study.best_value)
fig_importance = optuna.visualization.plot_param_importances(
    study, target=lambda t: t.duration.total_seconds(), target_name="duration")
...
"""
cb_params_best = {'learning_rate': 0.13948861596733592,
                  'l2_leaf_reg': 88.18802352033121,
                  'bagging_temperature': 1.0210260597520087,
                  'random_strength': 1.2300217338005526,
                  'depth': 6, 'min_data_in_leaf': 299, 'n_estimators': 857,'random_seed': 42}
#According to Ortal
base_params = {"objective":"binary",'max_depth':5,
...
final_cb.set_params(**cb_params_best)

base_cb = cb.CatBoostClassifier(random_seed = 42)
#base_cb.set_params(**base_params)

final_cb.fit(X_train,y_train)
base_cb.fit(X_train,y_train)

# Calcualte the balanced accuracy CB
tuned_acc_score = balanced_accuracy_score(y_test,final_cb.predict(X_test))
base_acc_score = balanced_accuracy_score(y_test,base_cb.predict(X_test))
print('Balanced accuracy for tuned model: ',round(tuned_acc_score,3))
print('Balanced accuracy for base model: ',round(base_acc_score,3))
print('An improvment of ',round(round(tuned_acc_score,3)-round(base_acc_score,3),3))
...
#________________________#
# AUC curves
base_pred_prob = base_cb.predict_proba(X_test)[:, 1]
tuned_pred_prob = final_cb.predict_proba(X_test)[:, 1]
# Compute ROC curve and AUC
fpr, tpr, _ = roc_curve(y_test, base_pred_prob)
roc_auc_baseline = auc(fpr, tpr)
...
plt.xlabel('1 - Speseficity')
plt.ylabel('Sensitivity')
plt.title('CB - GAN Balanced completely\nReceiver Operating Characteristic')
plt.legend(loc="lower right")
plt.savefig('roc_curve_GAN_fully_balanced.png',format='png', dpi=300,
           transparent=True)
plt.show()
